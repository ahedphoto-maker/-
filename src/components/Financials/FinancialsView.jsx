                ...
                {team.map(member => {
                  ...
                  return (
                    <tr key={member.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '12px', textAlign: 'left', fontWeight: 700, color: 'var(--primary-color)', fontFamily: 'Inter' }}>{formatCurrency(estimatedPay)}</td>
                      <td style={{ padding: '12px', textAlign: 'left', color: '#10b981', fontFamily: 'Inter' }}>{formatCurrency(paid)}</td>
                      <td style={{ padding: '12px', textAlign: 'left', color: remaining > 0 ? '#ef4444' : '#10b981', fontWeight: 800, fontFamily: 'Inter' }}>{formatCurrency(remaining)}</td>
                    </tr>
                  );
                })}
              </tbody>
              ...
              <tbody>
                {bookings.filter(b => b.bookingType === 'company').reduce((acc, curr) => {
                  ...
                  <tr key={idx} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    <td style={{ padding: '12px', textAlign: 'left', fontWeight: 700, fontFamily: 'Inter' }}>{formatCurrency(company.total)}</td>
                    <td style={{ padding: '12px', textAlign: 'left', color: '#10b981', fontFamily: 'Inter' }}>{formatCurrency(company.paid)}</td>
                    <td style={{ padding: '12px', textAlign: 'left', color: (company.total - company.paid) > 0 ? '#ef4444' : '#10b981', fontWeight: 800, fontFamily: 'Inter' }}>{formatCurrency(company.total - company.paid)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
        <div className="card" style={{ borderRight: '5px solid var(--primary-color)', padding: '16px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', transition: 'all 0.3s' }}>
          <div>
            ...
            <h3 style={{ fontSize: '1.5rem', fontWeight: 800, marginTop: '6px', fontFamily: 'Inter' }}>{formatCurrency(totalInvoiced)}</h3>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', borderTop: '1px solid var(--border-color)', paddingTop: '8px' }}>
            ...
            <span style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--status-warning)' }}>{formatCurrency(outstandingAmount)}</span>
          </div>
        </div>
        <div className="card" style={{ borderRight: '5px solid #10b981', padding: '16px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', transition: 'all 0.3s' }}>
          <div>
            ...
            <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#10b981', marginTop: '6px', fontFamily: 'Inter' }}>{formatCurrency(totalPaid)}</h3>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', borderTop: '1px solid var(--border-color)', paddingTop: '8px' }}>
        ...
        <div className="card" style={{ borderRight: '5px solid #ef4444', padding: '16px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', transition: 'all 0.3s' }}>
          <div>
            ...
            <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#ef4444', marginTop: '6px', fontFamily: 'Inter' }}>{formatCurrency(totalExpensesAmount)}</h3>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', borderTop: '1px solid var(--border-color)', paddingTop: '8px' }}>
        ...
        <div className="card" style={{ borderRight: '5px solid #8b5cf6', padding: '16px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', transition: 'all 0.3s' }}>
          <div>
            ...
            <h3 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#8b5cf6', marginTop: '6px', fontFamily: 'Inter' }}>{formatCurrency(netProfit)}</h3>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', borderTop: '1px solid var(--border-color)', paddingTop: '8px' }}>
                  ...
                  filteredInvoices.map(inv => {
                    ...
                    return (
                      <tr key={inv.id} style={{ borderBottom: '1px solid var(--border-color)', transition: 'background-color 0.2s' }} className="table-row-hover">
                        ...
                        <td style={{ padding: '14px' }}><strong>{formatCurrency(inv.total || 0)}</strong></td>
                        <td style={{ padding: '14px', color: '#10b981', fontWeight: 700 }}>{formatCurrency(inv.paid || 0)}</td>
                        <td style={{ padding: '14px', color: remaining > 0 ? 'var(--status-warning)' : 'var(--text-muted)', fontWeight: 700 }}>{formatCurrency(remaining)}</td>
                ...
                {filteredPayments.length > 0 ? (
                  filteredPayments.map(p => (
                    <tr key={p.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '14px' }}><strong style={{ color: '#10b981' }}>{formatCurrency(p.amount || 0)}</strong></td>
                ...
                {filteredExpenses.length > 0 ? (
                  filteredExpenses.map(ex => (
                    <tr key={ex.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '14px' }}><strong style={{ color: '#ef4444' }}>{formatCurrency(ex.amount || 0)}</strong></td>
                    ...
                    {expenseChartData.map((item, index) => {
                      ...
                      return (
                        <div key={index} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.8rem' }}>
                          ...
                          <span style={{ fontWeight: 700, color: 'var(--text-muted)' }}>{percentage}% ({formatCurrency(item.value)})</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <tbody>
                  {clients.map(c => (
                    <tr key={c.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '10px', fontWeight: 700, color: '#10b981' }}>{formatCurrency(c.totalSpent)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                    ...
                    >
                      ...
                      {bookings.map(b => (
                        <option key={b.id} value={b.id}>
                          {formatBookingNumber(b.bookingNumber)} - {b.title} ({formatCurrency(b.totalPrice)})
                        </option>
                      ))}
                    </select>
                  </div>
                    ...
                    {invoices.map(inv => {
                      ...
                      return (
                        <option key={inv.id} value={inv.invoiceNumber}>
                          {inv.invoiceNumber} - {inv.clientName} (المتبقي: {formatCurrency(remaining)})
                        </option>
                      );
                    })}
                  </select>
